...
def run_policy(env, policy, scaler, logger, plotter, episodes, plot=True):
    ...
    Everytime we call this method will trigger 50 episodes of training, hence getting 50 trajectories in total. Every
    trajectory returns a dict object containing lists of observes, actions, rewards, and unscaled_obs. append these 50 trajectory lead to a big
...
def run_episode(env, policy, scaler, animate=False):
    ...
    while not done:
        ...
        obs_new, reward, done, _ = env.step(action.reshape(-1))
...
def compute_vvalue(trajectories, val_func):
    ...
    Comment: should we emphasize that this value_func is trained on policy?
    """
...
def critic_compute_vvalue(dict_states, val_func):
    ...
    This is a intermediate step to compute the TD error. As the input shape is different we can't use the same one to predict.

    comment: A deterministic critic is actually a Max Q function: a state-action value function corresponding
    Also this critic is off policy as experience sampled from the buffer only correspond to old policies
    """
...
def build_train_set(trajectories):
    ...
    # TODO：check if it is something wrong for computing learning sigals
...
def TD(env, dict_states, policy, critic, gamma=0.995):
    ...
    """Compute 1 step temporal difference error.
    ...
    for state in states:
        ...
        state_, reward, done, _ = env.step(action.reshape(-1))
        state_ = np.append(state_, [state[-1]+0.001])
        states_.append(state_)
        rewards_.append(reward)
    # compute one-step forward values
