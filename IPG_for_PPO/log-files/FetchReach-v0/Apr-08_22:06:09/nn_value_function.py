...
class ValueFncNN(object):
    ...
    def fit(self, x, y, logger, plotter, id):
        ...
        """
        Using a real buffer for experience replay.
        Maintaining a queue for previous experience may also be experimented.
        """
        """
                Using a real buffer for experience replay.
                """
        ...
        elif self.replay_buffer_x.shape[0] > 2000:
            self.replay_buffer_x = np.concatenate([x, self.replay_buffer_x])[:2000]
            self.replay_buffer_y = np.concatenate([y, self.replay_buffer_y])[:2000]
        else:
